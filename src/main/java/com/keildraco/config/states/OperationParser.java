...
public class OperationParser extends AbstractParserBase implements IStateParser {
	...
	public OperationParser(TypeFactory factoryIn, ParserInternalTypeBase parentIn) {
	...
	public ParserInternalTypeBase getState(Tokenizer tok) throws IllegalParserStateException, UnknownStateException, GenericParseException {
		...
		case TILDE:
			...
			break;
		case IDENTIFIER:
			throw new GenericParseException("Found an Identifier where an Operator was expected");
		default:
			throw new GenericParseException(String.format("Found %s where an Operator was expected", operT.getValue()));
		}
		operT = tok.nextToken();
		if(!operT.getType().equals(TokenType.IDENTIFIER)) {
			throw new GenericParseException(String.format("Found %s where an Identifier was expected", operT.getValue()));
		}
		value = operT.getValue();
		operT = tok.nextToken();
		if(operT.getType().equals(TokenType.CLOSE_PARENS)) {
			OperationType rv = (OperationType) this.factory.getType(null, key, value, ItemType.OPERATION);
			rv.setOperation(oper);
			return rv;
		}
		throw new GenericParseException("Found "+operT.getValue()+" where a closing parenthesis was expected");
	}
